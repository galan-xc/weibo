...
class RedisPipeline(object):
    ...
    def process_item(self, item, spider):
        item_dict = dict(item)
        spider.logger.info("redis item-> {}".format(item_dict))
        data = {
            "id": item_dict["weibo"]["id"],
            "comments_count": int(item_dict["weibo"]["comments_count"]),
            "attitudes_count": int(item_dict["weibo"]["attitudes_count"]),
            "reposts_count": int(item_dict["weibo"]["reposts_count"]),
        }
...
class MysqlPipeline(object):
    ...
    def open_spider(self, spider):
        try:
            ...
            spider.logger.error("ImportError-> {}".format(e))
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            self.db.commit()
            spider.logger.debug("sql execute-> {}".format(ret))
