...
class Sina:
    ...
    def __login(self):
        ...
        driver = webdriver.Chrome(r"D:\customPath\chromedriver.exe")
        login = driver.get("https://weibo.com")
        # wait 10 seconds if timeout this method will fail
        time.sleep(10)
        print("开始模拟登陆登录")
        driver.find_element_by_xpath('//li/a[@node-type="loginBtn"]').click()
        time.sleep(2)
        print("开始输密码")
        driver.find_element_by_xpath('//div[@node-type="username_box"]/input').click()
        driver.find_element_by_xpath('//div[@node-type="username_box"]/input').send_keys(self.username)
        driver.find_element_by_xpath('//div[@node-type="password_box"]/input').click()
        driver.find_element_by_xpath('//div[@node-type="password_box"]/input').send_keys(self.password)
        # driver.find_element_by_css_selector(".password > div:nth-child(1) > input:nth-child(1)").click()
        # driver.find_element_by_css_selector(".password > div:nth-child(1) > input:nth-child(1)").send_keys(
        #     self.password)
        driver.find_element_by_xpath('//div[@class="item_btn"]/a[@node-type="submitBtn"]').click()
    ...
    def get_hot_search(self, keyword):
        ...
        if driver:
            ...
            for tag in tag_list:
                ...
                content = "".join(tag.xpath(
                    './/div[contains(@class,"feed_content")]/p[@class="comment_txt"]/text() | .//div[contains(@class,"feed_content")]/p[@class="comment_txt"]/em/text()'))
